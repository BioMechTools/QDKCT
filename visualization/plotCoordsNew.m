function [ hO ] = plotCoordsNew(varargin)
...
if nargin > 0
    ...
    if nnz(axisI) ==0 | nargin==3
        axisI = cellfun(@numel,varargin)==9 ;
        struct_MinMax = varargin{3};
    end
    if nnz(axisI)==1
        axes = varargin{axisI} ;
...
for di = 1 : numel(fnms)
    ...
    axis([struct_MinMax.xmin,struct_MinMax.xmax,struct_MinMax.ymin,struct_MinMax.ymax,struct_MinMax.zmin,struct_MinMax.zmax]);
